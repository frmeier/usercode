...
void dataPlotsMu(std::string fullPath)
{
    const std::string outpdf("dataPlotsMuplots");
    ...
    // Do plots
    const double etaLo = 0, etaHi = 2.6;

    const int nBins(13);

    // create an additional cut fo3ddr the mlb window
    Cuts mlbcut;
    ...
    TFile *fout = new TFile("histosdataPlotsMu.root","recreate");

    canvaspager(c,outpdf,nPads,canvasCdCounter,canvasPageCounter);
    c->cd(canvasCdCounter);
    doPlot1d(t,"hetamu", "TMath::Abs(reta1m)", (cutsAnalysis+"&&"+mlbcut.getCut()).c_str(),nBins,etaLo,etaHi,"","#eta(#mu_{1})","");
    c->Update();

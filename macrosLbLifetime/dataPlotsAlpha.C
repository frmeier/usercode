...
void dataPlotsAlpha(std::string fullPath)
{
    const std::string outpdf("dataPlotsAlphaplots");
    ...
    // Do plots
    const double alphal0lo(0), alphal0hi(0.030);
    const double alphalblo(0), alphalbhi(0.60);
    ...
    TFile *fout = new TFile("histosdataPlotsAlpha.root","recreate");

    canvaspager(c,outpdf,nPads,canvasCdCounter,canvasPageCounter);
    c->cd(canvasCdCounter);
    doPlot1d(t,"halphal0", "alphal0", (cuts.cs.getCutExceptOne(cuts.parvec,cuts.cs.getCutPos("alphal0"))).c_str(),nBins,alphal0lo,alphal0hi,"","#alpha(#Lambda^{0})","rad");
    c->Update();

    canvaspager(c,outpdf,nPads,canvasCdCounter,canvasPageCounter);
    c->cd(canvasCdCounter);
    doPlot1d(t,"halphalb", "alphalb", (cuts.cs.getCutExceptOne(cuts.parvec,cuts.cs.getCutPos("alphalb"))).c_str(),nBins,alphalblo,alphalbhi,"","#alpha(#Lambda_{b})","rad");
    c->Update();

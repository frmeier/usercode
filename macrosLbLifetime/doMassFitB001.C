...
struct doMassFitB001_fitresults
{
    double sig;
    double sig_err;
    double bgr;
...
// Unbinned likelihood fit for B0 mass using a simple Gaussian
doMassFitB001_fitresults doMassFitB001(TTree * tree, const double lumi, const string addTitle = "", const bool noTitle = false, const bool prelim = true, const bool noStat = false)
{
    cout << "-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-" << endl;
    cout << "This is doMassFitB001 -- fitting the mass of B0 to the passed tree" << endl;

    const double curEntries = tree->GetEntries();
    cout << "curEntries: " << curEntries << endl;

    // main configs are here
    //const double valLo(5.0075), valHi(5.5525);   // range to cover for fit and histo
    //const double binwidth(0.025);             // bin width for histo (not fit)
    const double valLo(5.16), valHi(5.40);   // range to cover for fit and histo
    ...
    model.fitTo(data,Extended(kTRUE), RooFit::NumCPU(fnCPU_B0_));

    RooPlot* xframe = mass.frame(Name("xframe"),Title(("Mass of B^{0}"+ (addTitle.size()>0 ? " - " + addTitle : "")).c_str()));
...
doMassFitB001_fitresults doMassFitB001_DG(TTree * tree, const double lumi, const string addTitle = "", const bool noTitle = false, const bool prelim = true, const bool noStat = false, const bool drawSigmaRanges = false)
{
    doMassFitB001_fitresults res; // container for results

    cout << "-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-" << endl;
    cout << "This is doMassFitB001 -- fitting the mass of B0 to the passed tree with a double Gaussian" << endl;
    ...
    //const double valLo(5.0075), valHi(5.5525);   // range to cover for fit and histo
    //const double valLo(5.16), valHi(5.40);   // range to cover for fit and histo
    const double valLo(5.14), valHi(5.50);   // range to cover for fit and histo
    const double binwidth(0.005);             // bin width for histo (not fit)
    const int nBins = (valHi-valLo)/binwidth; // width of the bins
    //const double sigmas(2.5);                 // region around peak to use for signal estimation
    ...
    RooRealVar sigma2("sigma2","sigma2",0.020,0.012,0.040);
...
// variant with crystal ball function
doMassFitB001_fitresults doMassFitB001_CB(TTree * tree, const double lumi, const string addTitle = "", const bool noTitle = false, const bool prelim = true, const bool noStat = false)
{
    cout << "-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-" << endl;
    cout << "This is doMassFitB001 -- fitting the mass of B0 to the passed tree" << endl;
    ...
    //const double binwidth(0.025);             // bin width for histo (not fit)
    const double valLo(5.0075), valHi(5.5525);   // range to cover for fit and histo
    //const double valLo(5.16), valHi(5.55);   // range to cover for fit and histo
    const double binwidth(0.005);             // bin width for histo (not fit)
    const int nBins = (valHi-valLo)/binwidth; // width of the bins
    const double sigmas(2.5);                 // region around peak to use for signal estimation
    ...
    //   mass fit
    RooRealVar mean_cb("mean_cb","mean",5.28,5.26,5.30);
    RooRealVar sigma_cb("sigma_cb","sigma",0.014,0.01,0.03);
    RooRealVar alpha_cb("alpha_cb","alpha_cb",1,0,5);
    RooRealVar n_cb("n_cb","n_cb",1.0,0.5,100.0);
    RooCBShape sig("sig","signal p.d.f.", mass, mean_cb, sigma_cb, alpha_cb, n_cb);
    ...
    Double_t m=mean_cb.getVal();
    Double_t m_err=mean_cb.getError();
    Double_t s1=sigma_cb.getVal();
    Double_t s1_err=sigma_cb.getError();
	...
	xframe->addObject(writeTLatex("mass: " + roundToString(m,m_err>0.003?3:4) + " #pm " + roundToString(m_err,m_err>0.003?3:4)+" GeV/c^{2}", txtPosLeft,txtPosTop-4*txtLineSpace));
	xframe->addObject(writeTLatex("width: " + roundToString(s1,s1_err>0.003?3:4) + " #pm " + roundToString(s1_err,s1_err>0.003?3:4)+ " GeV/c^{2}",txtPosLeft,txtPosTop-5*txtLineSpace));
	xframe->addObject(writeTLatex("alpha: " + roundToString(alpha_cb.getVal(),4) + " #pm " + roundToString(alpha_cb.getError(),4)+ " GeV/c^{2}",txtPosLeft,txtPosTop-6*txtLineSpace));
	xframe->addObject(writeTLatex("n: " + roundToString(n_cb.getVal(),4) + " #pm " + roundToString(n_cb.getError(),4)+ " ",txtPosLeft,txtPosTop-7*txtLineSpace));
    }
    if (lumi>0) xframe->addObject(writeTLatex("#int Ldt = " + toString(lumi) + " pb^{-1}",txtPosLeft,.80,textsize));
    if (prelim) xframe->addObject(writeTLatex("Work in progress",.18,.85,textsize));
    xframe->addObject(writeTLatex("#sqrt{s} = 7 TeV",.18,.77,textsize));
